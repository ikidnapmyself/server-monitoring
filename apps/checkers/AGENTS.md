...
- `apps/checkers/checkers/` — checker implementations
  ...
  - Some checkers (for example, `disk_macos`, `disk_linux`, `raid`, `disk_temp`, `cpu_temp`, `io_strain`) are OS-specific and may use platform gating — early return OK with a skip message on unsupported OSes (`raid` reads Linux `/proc/mdstat`; `disk_temp`/`cpu_temp` read Linux hwmon via `psutil.sensors_temperatures()`; `io_strain` samples `psutil.disk_io_counters` `busy_time` — all skip as OK on non-linux / when the data is unavailable)
