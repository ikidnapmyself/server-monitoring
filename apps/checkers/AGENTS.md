...
- `apps/checkers/checkers/` — checker implementations
  ...
  - Some checkers (for example, `disk_macos`, `disk_linux`, `raid`, `disk_temp`) are OS-specific and may use platform gating — early return OK with a skip message on unsupported OSes (`raid` reads Linux `/proc/mdstat`; `disk_temp` reads Linux hwmon via `psutil.sensors_temperatures()` and skips as OK on non-linux / when no disk sensors are present)
