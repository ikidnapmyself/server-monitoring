...
- `apps/checkers/checkers/` — checker implementations
  ...
  - Some checkers (for example, `disk_macos`, `disk_linux`, `raid`, `disk_temp`, `cpu_temp`, `listening_ports`) are OS-specific and may use platform gating — early return OK with a skip message on unsupported OSes (`raid` reads Linux `/proc/mdstat`; `disk_temp`/`cpu_temp` read Linux hwmon via `psutil.sensors_temperatures()`; `listening_ports` reads `psutil.net_connections` (`/proc/net`, no root on Linux only) — all skip as OK on non-linux / when the data is unavailable)
