...
class NotificationChannelAdmin(admin.ModelAdmin):
    ...
    def _lanes(self, obj):
        """Lanes bound to this channel, cheapest query first.

        Imported here rather than at module scope: ``apps.orchestration`` imports
        notify's drivers, so a top-level import closes a cycle.
        """
        from apps.orchestration.models import PipelineDefinition

        if obj.pk is None:
            return []
        return list(PipelineDefinition.objects.filter(channel_id=obj.pk).order_by("priority", "id"))

    @admin.display(description="Lanes")
    def lane_count(self, obj):
        """How many lanes route here, linked to exactly those lanes."""
        from apps.orchestration.models import PipelineDefinition

        count = len(self._lanes(obj))
        if not count:
            return count
        return changelist_link(PipelineDefinition, count, channel__id__exact=obj.pk)
