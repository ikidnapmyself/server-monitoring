    ...
    def execute(self, ctx: NodeContext, config: Dict[str, Any]) -> NodeResult:
        ...
        with track_duration(result):
            ...
            if not channels:
                ...
                if channel_obj:
                    ...
                    result.errors.append(
                        f"No active NotificationChannel found for drivers: {drivers}"
                    )
                    return result
            ...
            for channel in channels:
                ...
                if not driver_cls:
                    logger.warning(
                        "Unknown driver type: %s for channel %s",
                        channel.driver,
                        channel.name,
                    )
                ...
                try:
                    ...
                    logger.exception(
                        "Error sending via %s channel %s", channel.driver, channel.name
                    )
                    channels_failed += 1
                    deliveries.append(
                        {
    ...
    def _build_message(self, ctx: NodeContext, config: Dict[str, Any]):
        ...
        for node_id, output in ctx.previous_outputs.items():
            ...
            if "checks_run" in output:
                ...
                section = (
                    f"**Health Checks** "
                    f"({output.get('checks_passed', 0)}/{output.get('checks_run', 0)} passed)"
                )
                if failed_checks:
                    section += "\n" + "\n".join(failed_checks)
                if ok_checks:
