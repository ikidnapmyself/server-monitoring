    ...
    while true; do
        ...
        then
            case $TUIN_REPLY in
                ...
                "Run pipeline by definition")
                    raw=$(tuin_spin "Loading definitions" -- "$UV_BIN" run python manage.py show_pipeline --all 2>/dev/null) || true
                    defs=()
                    ...
                    if [ "${#defs[@]}" -eq 0 ]; then
                        ...
                        confirm_and_run "$UV_BIN run python manage.py run_pipeline --definition $pipeline_name"
                    fi ;;
                "Run pipeline from file")
                ...
                "List pipeline definitions")
                    confirm_and_run "$UV_BIN run python manage.py show_pipeline --all" ;;
                "Show one pipeline definition")
                    raw=$(tuin_spin "Loading definitions" -- "$UV_BIN" run python manage.py show_pipeline --all 2>/dev/null) || true
                    defs=()
                    while IFS= read -r _line; do
                    ...
                    if [ "${#defs[@]}" -eq 0 ]; then
                        ...
                        confirm_and_run "$UV_BIN run python manage.py show_pipeline --name $pipeline_name"
                    fi ;;
                "List recent pipeline runs")
