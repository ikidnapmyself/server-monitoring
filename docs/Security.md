...
- **Resolve external executables to a validated absolute path and invoke that — never a bare name resolved through `$PATH`.** `bin/lib/paths.sh` resolves `uv` once to `$UV_BIN`: it honors an explicit per-deployment `UV_BIN`, otherwise probes the standard absolute install locations — `~/.local/bin`, `~/.cargo/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin` (never `$PATH`) — requires an absolute path, and refuses a group/world-writable binary (tamper guard, following symlinks to the real target). A uv installed elsewhere is handled by pinning `UV_BIN` explicitly. A deploy runs with the application user's privileges, so a bare `uv` resolved on an attacker-influenced `$PATH` — or a `uv` planted earlier on it — would execute arbitrary code inside the deploy. Apply the same rule to any other tool the scripts shell out to.
